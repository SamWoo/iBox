...
public class ModelSettingFragment extends BaseLazyFragment {
    ...
    protected void init() {
        ...
        tvApi.setText(Hawk.get(HawkConfig.API_URL, HawkConfig.DEFAULT_API));
...
//            Log.e("Sam", "path----->" + requireActivity().getFilesDir().getAbsolutePath());
            ...
            if (!HawkConfig.WALLPAPER_API.isEmpty())
                OkGo.<File>get(HawkConfig.WALLPAPER_API).execute(new FileCallback(requireActivity().getFilesDir().getAbsolutePath(), "wp") {
                    @Override
        ...
        findViewById(R.id.epgApi).setOnClickListener(v -> {
            FastClickCheckUtil.check(v);
            ...
            dialog.setOnDismissListener(dialog12 -> {
                ((BaseActivity) mActivity).hideSysBar();
                EventBus.getDefault().unregister(dialog12);
            });
            dialog.show();
        });
        ...
        findViewById(R.id.llHistoryNum).setOnClickListener(v -> {
            FastClickCheckUtil.check(v);
        ...
        findViewById(R.id.showFastSearch).setOnClickListener(v -> {
            FastClickCheckUtil.check(v);
            Hawk.put(HawkConfig.FAST_SEARCH_MODE, !Hawk.get(HawkConfig.FAST_SEARCH_MODE, false));
            tvFastSearchText.setText(Hawk.get(HawkConfig.FAST_SEARCH_MODE, false) ? "已开启" : "已关闭");
        });
        findViewById(R.id.llHomeRecStyle).setOnClickListener(v -> {
            FastClickCheckUtil.check(v);
            Hawk.put(HawkConfig.HOME_REC_STYLE, !Hawk.get(HawkConfig.HOME_REC_STYLE, false));
            tvRecStyleText.setText(Hawk.get(HawkConfig.HOME_REC_STYLE, false) ? "是" : "否");
        });

        findViewById(R.id.llSearchTv).setOnClickListener(view -> {
            ...
            loadingSearchRemoteTvDialog.setOnDismissListener(dialogInterface -> EventBus.getDefault().unregister(loadingSearchRemoteTvDialog));
            loadingSearchRemoteTvDialog.show();

            RemoteTVBox tv = new RemoteTVBox();
            remoteTvHostList = new ArrayList<>();
            foundRemoteTv = false;
            view.postDelayed(() -> new Thread(() -> RemoteTVBox.searchAvalible(tv.new Callback() {
                @Override
